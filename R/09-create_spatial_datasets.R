...
save_spatial_summaries = function(year, x, y){
  ...
  st_write(y, paste0("data/spatial/nlcc_", year, ".gpkg"), delete_dsn = TRUE)
}

all_summaries = read_csv("data/database/nlcc.csv")
...
save_spatial_changes = function(year_change, x, y){
  ...
  y = left_join(y, x, by = "NAME0")
  st_write(y, paste0("data/spatial/nlct_", year_change, ".gpkg"), delete_dsn = TRUE)
}

all_changes = read_csv("data/database/nlct.csv")
